...
int main(void) {
  ...
  while(1) {
    ...
    if (sensor_read_flag) {
        sensor_read_flag = false; // Clear the flag

        // Read the sensor and update advertisement data
        light_state = opt3004_read_result();
        uint8_t* buf = (uint8_t*) &light_state;
        simple_ble_adv_manuf_data(buf, sizeof(float));

        // Print the updated sensor reading
        printf("Updated sensor reading: %f\n", light_state);
    }
  }
}
